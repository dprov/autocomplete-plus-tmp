...
class AutocompleteManager {
  ...
  initialize () {
    ...
    this.subscriptions.add(atom.config.observe('autocomplete-plus-tmp.enableExtendedUnicodeSupport', enableExtendedUnicodeSupport => {
  ...
  handleEvents () {
    ...
    this.subscriptions.add(atom.config.observe('autocomplete-plus-tmp.backspaceTriggersAutocomplete', (value) => { this.backspaceTriggersAutocomplete = value }))
    this.subscriptions.add(atom.config.observe('autocomplete-plus-tmp.enableAutoActivation', (value) => { this.autoActivationEnabled = value }))
    this.subscriptions.add(atom.config.observe('autocomplete-plus-tmp.enableAutoConfirmSingleSuggestion', (value) => { this.autoConfirmSingleSuggestionEnabled = value }))
    this.subscriptions.add(atom.config.observe('autocomplete-plus-tmp.consumeSuffix', (value) => { this.consumeSuffix = value }))
    this.subscriptions.add(atom.config.observe('autocomplete-plus-tmp.useAlternateScoring', (value) => { this.useAlternateScoring = value }))
    this.subscriptions.add(atom.config.observe('autocomplete-plus-tmp.fileBlacklist', (value) => {
      if (value) {
        this.fileBlacklist = value.map((s) => { return s.trim() })
      }
      this.isCurrentFileBlackListedCache = null
    }))
    this.subscriptions.add(atom.config.observe('autocomplete-plus-tmp.suppressActivationForEditorClasses', value => {
      this.suppressForClasses = []
  ...
  handleCommands () {
    return this.subscriptions.add(atom.commands.add('atom-text-editor', {
      'autocomplete-plus-tmp:activate': (event) => {
      ...
      'autocomplete-plus-tmp:navigate-to-description-more-link': () => {
...
See https://github.com/atom/autocomplete-plus-tmp/wiki/Provider-API`
      )
    }
...
See https://github.com/atom/autocomplete-plus-tmp/wiki/Provider-API`
  ...
  displaySuggestions (suggestions, options) {
    switch (atom.config.get('autocomplete-plus-tmp.similarSuggestionRemoval')) {
      case 'textOrSnippet': {
  ...
  requestNewSuggestions () {
    let delay = atom.config.get('autocomplete-plus-tmp.autoActivationDelay')
