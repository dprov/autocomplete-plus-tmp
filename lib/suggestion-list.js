...
class SuggestionList {
  ...
  initialize () {
    ...
    this.subscriptions.add(atom.commands.add('atom-text-editor.autocomplete-active', {
      'autocomplete-plus-tmp:confirm': this.confirmSelection,
      'autocomplete-plus-tmp:confirmIfNonDefault': this.confirmSelectionIfNonDefault,
      'autocomplete-plus-tmp:cancel': this.cancel
    }))
  ...
  addBindings (editor) {
    ...
    const completionKey = atom.config.get('autocomplete-plus-tmp.confirmCompletion') || ''

    const keys = {}
    if (completionKey.indexOf('tab') > -1) { keys['tab'] = 'autocomplete-plus-tmp:confirm' }
    if (completionKey.indexOf('enter') > -1) {
      if (completionKey.indexOf('always') > -1) {
        keys['enter'] = 'autocomplete-plus-tmp:confirmIfNonDefault'
      } else {
        keys['enter'] = 'autocomplete-plus-tmp:confirm'
      }
    }
    ...
    const useCoreMovementCommands = atom.config.get('autocomplete-plus-tmp.useCoreMovementCommands')
    const commandNamespace = useCoreMovementCommands ? 'core' : 'autocomplete-plus-tmp'
    ...
    return this.bindings.add(
      atom.config.onDidChange('autocomplete-plus-tmp.useCoreMovementCommands', () => {
  ...
  */
  ...
  show (editor, options) {
    if (atom.config.get('autocomplete-plus-tmp.suggestionListFollows') === 'Cursor') {
  ...
  showAtBeginningOfPrefix (editor, prefix, followRawPrefix = false) {
    ...
    if (this.activeEditor === editor) {
      ...
      if (editor) {
        ...
        this.overlayDecoration = editor.decorateMarker(marker, {type: 'overlay', item: this.suggestionListElement, position: 'tail', class: 'autocomplete-plus-tmp'})
  ...
  showAtCursorPosition (editor) {
    ...
    if (marker) {
      ...
      this.overlayDecoration = editor.decorateMarker(marker, {type: 'overlay', item: this.suggestionListElement, class: 'autocomplete-plus-tmp'})
      return this.addBindings(editor)
    }
  }
